                ...
                <div class="tab-content my-account-tab mt-30" id="pills-tabContent">
                    <div class="tab-pane fade active show">
                        <div class="my-account-order account-wrapper">
                            <table id="example" class="table table-striped table-bordered wishlist-table"
                                style="width:100%;">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    @foreach ($wishlist as $key => $wish)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ URL::to('/shop-single/' . $wish->idProduct) }}"><img
                                                        src="{{ asset('/storage/kidadmin/images/product/' . $image) }}"
                                                        alt=""></a>
                                            </td>
                                            <td>
                                                <div><a
                                                        href="{{ URL::to('/shop-single/' . $wish->idProduct) }}">{{ $wish->ProductName }}</a>
                                                </div>
                                            ...
                                            <td class="text-center">
                                                <a class="view-hover h3 d-block delete-wish" data-id="{{ $wish->idWish }}"
                                                    data-toggle="tooltip" data-placement="top" title=""
                                                    data-original-title="Xóa"><i class="fa fa-trash"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                ...
                if (confirm('Bạn có chắc chắn muốn xóa sản phẩm yêu thích này không?')) {
                    $.ajax({
                        ...
                        success: function(response) {
                            // Xóa hàng liên quan nếu trong bảng
